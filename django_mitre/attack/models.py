...
class Group(BaseModel, DescriptiveMixIn, MitreIdentifiableMixIn):
    """Group in Mitre Att&ck data and Intrusion Set in STIX 2.1

    Uses the ``intrusion-set`` STIX 2.1 data type.

    """

    stix_data_types = ("intrusion-set",)
...
class DataSource(BaseModel, DescriptiveMixIn, MitreIdentifiableMixIn):
    ...
    # `datacomponent_set` is the reverse relationship
    platforms = models.JSONField(null=True, blank=True)
    version = models.CharField(max_length=16)
    contributors = models.JSONField(null=True, blank=True)
...
class Mitigation(BaseModel, DescriptiveMixIn, MitreIdentifiableMixIn):
    ...
    Uses the ``course-of-action`` STIX 2.1 data type.

    """

    stix_data_types = ("course-of-action",)
...
class Identity(BaseModel):
    """Identity in STIX 2.1

    Uses the ``identity`` STIX 2.1 data type.

    """

    stix_data_types = ("identity",)
    name = models.CharField(max_length=255)
    identity_class = models.CharField(max_length=255)


class Campaign(BaseModel, DescriptiveMixIn, MitreIdentifiableMixIn):
    """Campaign in Mitre Attack and STIX 2.1

    Uses the ``campaign`` STIX 2.1 data type.

    """

    stix_data_types = ("campaign",)
    aliases = models.JSONField(blank=True, null=True)
