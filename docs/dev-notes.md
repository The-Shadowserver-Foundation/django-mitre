...
Versions of the data are essentially updates and additions to entries. We can therefore assume with some certainty that relationships built within one version are retained in the next.
...
We get updates from this source when MITRE pushes a release. This gives us a stable source of the data MITRE ATT&CK produces. More information about the data format can be found in the [ATT&CK Data Model - STIX Schemas](https://mitre-attack.github.io/attack-data-model/schemas/) documentation.
...
The data is arranged by three main categories that are versioned collections (in the STIX sense).
...
Please refer to the [ATT&CK Data Model - STIX Schemas](https://mitre-attack.github.io/attack-data-model/schemas/) for information about how the data is modeled.
...
List each tactic with the number of techniques and techniques with sub-techniques:
